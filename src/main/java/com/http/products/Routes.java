...
public class Routes {
	...
	public static void buildRoutes(Router router, Vertx vertx) {
		...
		// create product
		router.put("/product/create/").handler(rh -> {
	...
	private static void logRouteMeta(String vid, RoutingContext rh) {
		...
		if (!rh.cookies().contains(SESSION_LOGIN_KEY)) {
			System.out.println("request:" + vid + ":" + " No session key found !!");
		}
	}
}
